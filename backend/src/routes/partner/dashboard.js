  ...
  try {
    ...
    const stats = await sequelize.query(`
      SELECT
        ...
        COALESCE(SUM("productionCostAmount"), 0) as production_cost_amount,
    ...
    res.json({
      ...
      summary: {
        ...
        grossRevenue: parseFloat(stats[0].gross_amount || 0),
        productionCost: parseFloat(stats[0].production_cost_amount || 0),
        profitAmount: parseFloat(stats[0].profit_amount || 0),
        partnerEarning: parseFloat(stats[0].partner_earning || 0),
        companyEarning: parseFloat(stats[0].company_earning || 0),
  ...
  try {
    ...
    const performance = await sequelize.query(`
      SELECT
        ...
        COALESCE(SUM(t."grossAmount"), 0) as total_gross_amount
      FROM locations l
  ...
  try {
    ...
    const trends = await sequelize.query(`
      SELECT
        ...
        COALESCE(SUM("grossAmount"), 0) as gross_revenue
      FROM transactions
      WHERE "partnerId" = :partnerId
        AND status = 'Completed'
        AND "stopTime" >= :startDate
    ...
    trends.forEach(row => {
      ...
      if (dateMap[dateStr]) {
        ...
        dateMap[dateStr].grossRevenue = parseFloat(row.gross_revenue || 0);
      }
    });
...
router.get('/notifications', authenticate, partnerOnly, async (req, res) => {
  try {
    ...
    const recent = await sequelize.query(`
      SELECT t."transactionId", t."partnerEarning", t."stopTime"
      FROM transactions t
      WHERE t."partnerId" = :partnerId
        AND t.status = 'Completed'
      ORDER BY t."stopTime" DESC
    ...
    if (recent.length > 0) {
      notifications.push({
        ...
        message: `Earned ₦${parseFloat(recent[0].partnerEarning || 0).toLocaleString()} from latest charging session`,
