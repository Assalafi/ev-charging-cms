...
/**
 * @route   GET /api/ocpp/messages
 * @desc    Get OCPP messages for a charging station with pagination
 * @access  Private
 */
router.get('/messages', authenticate, async (req, res) => {
  try {
    ...
    // Query total count
    const count = await OcppMessage.count({
      where: { chargePointId }
    });
    
    // Query messages with pagination
    const messages = await OcppMessage.findAll({
      where: { chargePointId },
    ...
    // Log for debugging
    logger.debug(`Found ${messages.length} OCPP messages for station ${chargePointId} (page ${page}, limit ${limit})`); 
    ...
    logger.error('Error fetching OCPP messages:', error);
    res.status(500).json({
      success: false,
      message: 'Server error'
    });
