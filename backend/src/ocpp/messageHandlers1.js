...
async function handleAuthorize(chargePointId, uniqueId, payload) {
    try {
        logger.info(`Processing Authorize from ${chargePointId}: ${JSON.stringify(payload)}`);
        ...
        logger.info(`Authorization result for ${idTag}: ${JSON.stringify(authResult)}`);
...
async function handleStartTransaction(chargePointId, uniqueId, payload) {
    try {
        ...
        try {
            ...
            try {
                await updateConnectorStatus(chargePointId, normalizedPayload.connectorId, 'Charging', transaction.transactionId);
                logger.info(`Updated connector ${chargePointId}:${normalizedPayload.connectorId} status to Charging with transaction ${transaction.transactionId}`);
            } catch (connError) {
            ...
            if (chargingSessionTracker) {
                try {
                    chargingSessionTracker.startSession(transaction.transactionId, [], normalizedPayload.meterStart);
                    logger.info(`Started charging session tracking for transaction ${transaction.transactionId}`);
                } catch (trackerError) {
                    logger.error(`Error starting charging session tracker: ${trackerError.message}`);
                }
            }
            ...
            if (mqttClient) {
                try {
                    mqttClient.publish(`ocpp/transactions/${transaction.transactionId}/start`, JSON.stringify({
                    ...
                    mqttClient.publish(`ocpp/stations/${chargePointId}/energy`, 
                        JSON.stringify({
                            timestamp: new Date().toISOString(),
                            energy: 0,
...
async function handleStopTransaction(chargePointId, uniqueId, payload) {
    try {
        ...
        try {
            ...
            const energyDelivered = Math.max(0, normalizedPayload.meterStop - startMeterValue);
            ...
            await updateConnectorStatus(chargePointId, transaction.connectorId, 'Finishing', null);
            ...
            logger.info(`Completed transaction ${transaction.transactionId} for ${chargePointId} with energy delivered ${energyDelivered} Wh and amount ${transactionAmount}`);
            
            // Stop the charging session tracking
            if (chargingSessionTracker) {
                chargingSessionTracker.endSession(normalizedPayload.transactionId, normalizedPayload.meterStop);
                logger.info(`Stopped real-time tracking for transaction ${normalizedPayload.transactionId}`);
            }
            ...
            logger.info(`Emitted stop-transaction event for transaction ${normalizedPayload.transactionId}`);
            ...
            if (mqttClient) {
                ...
                mqttClient.publish(`ocpp/transactions/${normalizedPayload.transactionId}/stop`, 
                    JSON.stringify({
                ...
                setTimeout(() => {
                    ...
                    updateConnectorStatus(chargePointId, transaction.connectorId, 'Available', null)
                        .catch(error => logger.error(`Failed to update connector status to Available: ${error.message}`));
        ...
        if (idTag) {
            try {
                ...
                logger.error(`Error checking authorization during StopTransaction:`, authError);
...
async function handleMeterValues(chargePointId, uniqueId, payload) {
    try {
        ...
        if (meterValues.length === 0) {
            logger.warn(`Empty meterValue array in MeterValues request from ${chargePointId}`);
        } else {
            ...
            try {
                for (const meterValue of meterValues) {
                    ...
                    for (const sampledValue of sampledValues) {
                        ...
                        if (sampledValue.measurand === 'Energy.Active.Import.Register' || !sampledValue.measurand) {
                            ...
                            if (!isNaN(energyValue)) {
                                ...
                                try {
                                    ...
                                    logger.debug(`Failed to update connector meter value: ${connectorError.message}`);
                                }
                                ...
                                if (transactionId) {
                                    try {
                                        ...
                                        if (transaction) {
                                            const energyDelivered = Math.max(0, energyValue - transaction.startMeterValue);
                                            await transaction.update({
                                            ...
                                            if (mqttClient) {
                                                ...
                                                mqttClient.publish(`ocpp/transactions/${transactionId}/energy`, 
                                                    JSON.stringify({
                                                        timestamp,
                                        ...
                                        logger.warn(`Error updating transaction energy: ${txError.message}`);
                                    }
                                }
                            }
                        }
                ...
                logger.error(`Database error during MeterValues from ${chargePointId}:`, dbError);
            }
        }
...
async function updateConnectorStatus(chargePointId, connectorId, status, transactionId = null) {
    try {
        await sequelize.query(
    ...
    try {
        ...
        try {
            ...
            if (status === 'Charging') {
                ...
                if (transaction) {
                    logger.info(`Station ${chargePointId} connector ${connectorId} started charging transaction ${transaction.id}`);
                    
                    // Start tracking energy if not already started
                    if (chargingSessionTracker && !chargingSessionTracker.activeTransactions.has(transaction.id)) {
                        logger.info(`Starting energy tracking for transaction ${transaction.id}`);
                        chargingSessionTracker.startSession(transaction.id, [], transaction.startMeterValue);
                    }
                ...
                logger.info(`Station ${chargePointId} connector ${connectorId} is preparing to charge`);
                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (connector.length > 0 && connector[0].status === 'Preparing') {
                            logger.warn(`Station ${chargePointId} connector ${connectorId} stuck in Preparing state for 30s - resetting to Available`);
                            await updateConnectorStatus(chargePointId, connectorId, 'Available');
                        ...
                        logger.error(`Error handling Preparing timeout: ${timeoutError.message}`);
                    }
        ...
        logger.error(`Error handling StatusNotification from ${chargePointId}:`, error);
        // Return empty response even on error
        return [3, uniqueId, {}];
    }
}
