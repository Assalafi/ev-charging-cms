      ...
      try {
        for (const meterValue of meterValues) {
          ...
          for (const sampledValue of sampledValues) {
            ...
            if (sampledValue.measurand === "SoC" && sampledValue.unit === "Percent") {
              batteryPercentage = parseInt(sampledValue.value, 10);
              logger.debug(`Found SoC: ${batteryPercentage}% for transaction ${transactionId} on ${chargePointId}`);
            }
            
            // Check for Power readings
            if (sampledValue.measurand === "Power.Active.Import" && sampledValue.unit === "W") {
              powerValue = parseInt(sampledValue.value, 10);
              logger.debug(`Found Power: ${powerValue}W for transaction ${transactionId} on ${chargePointId}`);
            }
                  ...
                  try {
                    ...
                    if (transaction) {
                      ...
                      await transaction.update({
                        ...
                        batteryPercentage: batteryPercentage || transaction.batteryPercentage // Only update if we have a new value
                      })
                      ...
                      if (mqttClient) {
                        ...
                        mqttClient.publish(
                          ...
                          JSON.stringify({
                            ...
                            power: powerValue || Math.round(Math.random() * 7000) + 3000, // Use actual power or fallback to simulation
                            // Don't convert the price and amount fields - send raw calculated values
                            price: mqttPrice,
                            amount: mqttAmount,
                            chargePointId,
