...
async function logToDatabase(messageType, status, payload, response = null, errorCode = null) {
  try {
    await sequelize.query(
      `INSERT INTO ocpp_messages
        ("chargePointId", message_type, status, payload, "timestamp", "messageId", "errorCode")
       VALUES
        ($1, $2, $3, $4, $5, $6, $7)`,
      {
        bind: [
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (Array.isArray(response) && response[0] === 3) {
        ...
        logToDatabase(
          'RemoteStopTransaction',
          responseStatus === 'Accepted' ? 'Received' : 'Failed',
          { transactionId: TRANSACTION_ID },
          response[2],
          errorCode
        );
      ...
      // Close connection after handling response
      ws.close();
    } catch (error) {
