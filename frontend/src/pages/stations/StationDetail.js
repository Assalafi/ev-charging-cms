...
function StationDetail() {
  ...
  const fetchStationData = async () => {
    ...
    try {
      ...
      const stationResponse = await fetch(
        `http://localhost:3000/api/stations/${stationId}`,
        {
          headers: {
  ...
  const fetchOcppMessages = async (page = 0, limit = messagesLimit) => {
    ...
    try {
      ...
      const messagesResponse = await fetch(
        `http://localhost:3000/api/stations/${stationId}/logs?limit=${limit}&offset=${offset}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
    ...
    try {
      ...
      try {
        const activeResponse = await fetch(
          `http://localhost:3000/api/stations/${stationId}/transactions?status=InProgress&limit=1`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
      ...
      const transactionsResponse = await fetch(
        `http://localhost:3000/api/stations/${stationId}/transactions?limit=${limit}&offset=${offset}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      switch (commandType) {
        case "RemoteStart":
          response = await api.post(
            `/stations/${stationId}/remote-start`,
            commandParams,
          );
          break;
        case "RemoteStop":
          response = await api.post(
            `/stations/${stationId}/remote-stop`,
            commandParams,
          );
          break;
        case "Reset":
          response = await api.post(
            `/stations/${stationId}/reset`,
            commandParams,
          );
          break;
        case "ChangeAvailability":
          response = await api.post(
            `/stations/${stationId}/change-availability`,
            commandParams,
          );
          break;
        default:
  ...
  const checkConnectionStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/api/stations/${stationId}/connection`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchStationDetailsUpdate = async () => {
    ...
    try {
      ...
      const stationResponse = await fetch(
        `http://localhost:3000/api/stations/${stationId}/status`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
