...
const defaultSettlementPeriod = (periodType = 'monthly') => {
  const now = new Date();
...
const SettlementList = () => {
  ...
  const handleGenerateSettlement = async () => {
    if (!generateDialog.partnerId || !generateDialog.periodType ||
        !generateDialog.periodStart || !generateDialog.periodEnd) {
      setGenerateDialog(current => ({
        ...current,
        error: 'Partner, period type, start date and end date are required.'
      }));
      return;
    }
    if (generateDialog.periodStart > generateDialog.periodEnd) {
      setGenerateDialog(current => ({
        ...current,
        error: 'Period start must be before the period end.'
      }));
      return;
    }

    try {
      setGenerateDialog(current => ({ ...current, generating: true, error: '' }));
      ...
      if (response.data.success) {
        ...
        setGenerateDialog(current => ({
          ...current,
          generating: false,
          error: response.data.message || 'Settlement could not be generated.'
        }));
      }
    } catch (error) {
      setGenerateDialog(current => ({
        ...current,
        generating: false,
        error: error.response?.data?.message || error.message || 'Settlement could not be generated.'
      }));
    }
  };
  ...
  const openGenerateDialog = () => {
    ...
    setGenerateDialog({
      open: true,
      partnerId: filters.partnerId !== 'all' ? filters.partnerId : (partners[0]?.id || ''),
      periodType,
      ...defaultSettlementPeriod(periodType),
      generating: false,
      error: ''
    });
  };

  const changeSettlementPeriodType = (periodType) => {
    setGenerateDialog(current => ({
      ...current,
      periodType,
      ...defaultSettlementPeriod(periodType),
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow key={settlement.id}>
                    ...
                    <TableCell>₦{Number(settlement.grossAmount || 0).toLocaleString()}</TableCell>
                    <TableCell>₦{Number(settlement.productionCostAmount || 0).toLocaleString()}</TableCell>
                    <TableCell>₦{Number(settlement.adjustmentAmount || 0).toLocaleString()}</TableCell>
                    <TableCell>₦{Number(settlement.finalPayableAmount || settlement.partnerEarning || 0).toLocaleString()}</TableCell>
                    <TableCell>
                      <Chip
      ...
      <Dialog open={generateDialog.open} onClose={() => setGenerateDialog({ open: false })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {generateDialog.error && (
            <Alert severity="error" sx={{ mt: 2 }}>{generateDialog.error}</Alert>
          )}
          <FormControl fullWidth sx={{ mt: 2 }}>
            <InputLabel>Partner</InputLabel>
            <Select
              value={generateDialog.partnerId || ''}
              onChange={(e) => setGenerateDialog({ ...generateDialog, partnerId: e.target.value, error: '' })}
              label="Partner"
            >
              {partners.map((p) => (
                <MenuItem key={p.id} value={p.id}>{p.name}</MenuItem>
          ...
          <FormControl fullWidth sx={{ mt: 2 }}>
            ...
            <Select
              value={generateDialog.periodType || ''}
              onChange={(e) => changeSettlementPeriodType(e.target.value)}
              label="Period Type"
            >
          ...
          <TextField
            ...
            onChange={(e) => setGenerateDialog({ ...generateDialog, periodStart: e.target.value, error: '' })}
            sx={{ mt: 2 }}
          ...
          <TextField
            ...
            onChange={(e) => setGenerateDialog({ ...generateDialog, periodEnd: e.target.value, error: '' })}
            sx={{ mt: 2 }}
            InputLabelProps={{ shrink: true }}
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setGenerateDialog({ open: false })} disabled={generateDialog.generating}>Cancel</Button>
          <Button
            onClick={handleGenerateSettlement}
            variant="contained"
            disabled={generateDialog.generating || !generateDialog.partnerId}
          >
            {generateDialog.generating ? 'Generating…' : 'Generate'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={actionDialog.open} onClose={() => setActionDialog({ open: false, settlement: null, action: null })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {actionDialog.action === 'approve' && (
            <Typography>Are you sure you want to approve this settlement of ₦{Number(actionDialog.settlement?.finalPayableAmount || actionDialog.settlement?.partnerEarning || 0).toLocaleString()}?</Typography>
          )}
          {actionDialog.action === 'mark-paid' && (
            <>
              <Typography sx={{ mb: 2 }}>Mark settlement of ₦{Number(actionDialog.settlement?.finalPayableAmount || actionDialog.settlement?.partnerEarning || 0).toLocaleString()} as paid</Typography>
              <TextField
                fullWidth
