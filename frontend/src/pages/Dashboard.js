...
import PowerGridMonitor from '../components/PowerGridMonitor';
...
function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const energyResponse = await api.get('/transactions/stats/energy?period=day');
      ...
      setEnergyData({
        labels: energyResponse.data.energyStats.map(item => format(new Date(item.timestamp), 'HH:mm')),
        datasets: [
          {
            label: 'Energy (kWh)',
            data: energyResponse.data.energyStats.map(item => item.energy),
  ...
  return (
    <Box>
      ...
      {/* Power Grid Stability Component */}
      <PowerGridMonitor />
        ...
        <Grid item xs={12} sm={6} md={2}>
          <Paper elevation={2} sx={{ p: 2, height: '100%', borderRadius: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Box>
                <Typography variant="h5" component="div">
                  {(stats.energyToday || 0).toFixed(2)} kWh
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Energy Today
                </Typography>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          <Card sx={{ height: '100%', borderRadius: 2 }}>
            ...
            <CardContent>
              <Box sx={{ height: 300 }}>
                <Line data={energyData} options={lineChartOptions} />
              </Box>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Card sx={{ height: '100%', borderRadius: 2 }}>
            <CardHeader title="Station Usage" />
            <Divider />
            <CardContent>
              <Box sx={{ height: 300 }}>
                <Bar data={stationUsage} options={barChartOptions} />
              </Box>
            </CardContent>
          </Card>
        </Grid>
