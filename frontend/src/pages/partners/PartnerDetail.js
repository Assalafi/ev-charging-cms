...
const PartnerDetail = () => {
  ...
  const fetchAvailableLocations = async () => {
    try {
      ...
      const available = response.data.locations?.filter(
        location => !location.partnerId && !assignedIds.includes(location.id)
      ) || [];
  ...
  const handleAssignLocation = async () => {
    const location = availableLocations.find(item => Number(item.id) === Number(assignForm.locationId));
    const productionCost = assignForm.productionCostPerWh === ''
      ? undefined
      : Number(assignForm.productionCostPerWh);
    const partnerShare = assignForm.partnerSharePercent === ''
      ? undefined
      : Number(assignForm.partnerSharePercent);
    const costExceedsPrice = location && productionCost > Number(location.pricePerWh);
    if (costExceedsPrice && !window.confirm(
      'Production cost is above this location’s selling price, so sessions may produce no distributable profit. Assign anyway?'
    )) {
      return;
    }
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <Grid item xs={12} sm={3}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5">{formatNaira(partner.stats?.gross_amount)}</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={3}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5" color="success.main">
                {formatNaira(partner.stats?.total_partner_earning)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="h5" color="warning.main">
                {formatNaira(partner.pendingSettlement)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={3}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5" color="success.main">
                {formatNaira(partner.paidSettlement)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
      ...
      <Card>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <Box sx={{ display: 'grid', gap: 2 }}>
                ...
                <Box><Typography variant="body2" color="textSecondary">Default Production Cost:</Typography><Typography>{formatNaira(partner.defaultProductionCostPerWh)}/Wh</Typography></Box>
              </Box>
            </Grid>
            <Grid item xs={12} md={6}>
              ...
              <TableBody>
                {partner.locations?.length === 0 ? (
                  ...
                  <TableRow key={location.id}>
                    ...
                    <TableCell>{formatNaira(location.productionCostPerWh)}/Wh</TableCell>
                    <TableCell>
                      <Chip
        ...
        <Box sx={{ p: 3 }} hidden={tabValue !== 2}>
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {(partner.locations || []).flatMap(location =>
                  (location.stations || []).map(station => (
                    <TableRow key={station.id}>
                      <TableCell>{station.name}</TableCell>
                      <TableCell>{station.chargePointId}</TableCell>
                      <TableCell>{location.name}</TableCell>
                      <TableCell><Chip size="small" label={station.status || 'Unknown'} /></TableCell>
                    </TableRow>
                  ))
                )}
                {(partner.locations || []).every(location => !location.stations?.length) && (
                  <TableRow><TableCell colSpan={4} align="center">No stations assigned</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </TableContainer>
          ...
          <TableContainer>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell>Location</TableCell>
                  <TableCell align="right">Selling Price / Wh</TableCell>
                  <TableCell align="right">Production Cost / Wh</TableCell>
              ...
              <TableBody>
                {(partner.locations || []).map(location => (
                  <TableRow key={location.id}>
                    <TableCell>{location.name}</TableCell>
                    <TableCell align="right">{formatNaira(location.pricePerWh)}</TableCell>
                    <TableCell align="right">{formatNaira(location.productionCostPerWh)}</TableCell>
                    <TableCell align="right">{Number(location.partnerSharePercent || 0)}%</TableCell>
                    <TableCell>{location.settlementEnabled ? 'Enabled' : 'Disabled'}</TableCell>
                  </TableRow>
                ))}
                {!partner.locations?.length && (
                  <TableRow><TableCell colSpan={5} align="center">No location pricing configured</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </TableContainer>
        ...
        {/* Performance Tab */}
        <Box sx={{ p: 3 }} hidden={tabValue !== 4}>
          <Grid container spacing={3}>
            {[
              ['Completed transactions', partner.stats?.total_transactions || 0],
              ['Energy delivered', `${(Number(partner.stats?.total_energy_wh || 0) / 1000).toLocaleString()} kWh`],
              ['Gross revenue', formatNaira(partner.stats?.gross_amount)],
              ['Partner earning', formatNaira(partner.stats?.total_partner_earning)],
              ['Company earning', formatNaira(partner.stats?.total_company_earning)],
              ['Active stations', (partner.locations || []).flatMap(location => location.stations || [])
                .filter(station => !['Unavailable', 'Faulted', 'Offline'].includes(station.status)).length]
            ].map(([label, value]) => (
              ...
              <TableBody>
                {(partner.settlements || []).map(settlement => (
                  <TableRow key={settlement.id}>
                    ...
                    <TableCell align="right">{formatNaira(settlement.grossAmount)}</TableCell>
                    <TableCell align="right">{formatNaira(settlement.finalPayableAmount)}</TableCell>
                    <TableCell><Chip size="small" label={settlement.status} /></TableCell>
                  </TableRow>
                ))}
                {!partner.settlements?.length && (
                  <TableRow><TableCell colSpan={5} align="center">No settlements generated</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </TableContainer>
